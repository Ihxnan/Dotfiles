	...
	-- 3. Mason-LSP 关联（原配置不变，新增注释说明兼容性）
	{
		...
		config = function()
			...
			local on_attach = function(client, bufnr)
				...
				-- 显示代码动作
				vim.keymap.set("n", "<leader>ca", vim.lsp.buf.code_action, vim.tbl_extend("force", opts, { desc = "显示代码动作" }))
				-- 查看引用
				vim.keymap.set("n", "gr", vim.lsp.buf.references, vim.tbl_extend("force", opts, { desc = "查看引用" }))
				-- 诊断相关快捷键
				vim.keymap.set("n", "<leader>]", vim.diagnostic.goto_next, vim.tbl_extend("force", opts, { desc = "跳转到下一个诊断错误" }))
				vim.keymap.set("n", "<leader>[", vim.diagnostic.goto_prev, vim.tbl_extend("force", opts, { desc = "跳转到上一个诊断错误" }))
				vim.keymap.set("n", "<leader>dl", vim.diagnostic.setloclist, vim.tbl_extend("force", opts, { desc = "打开诊断列表" }))
				vim.keymap.set("n", "<leader>dh", vim.diagnostic.open_float, vim.tbl_extend("force", opts, { desc = "显示当前行诊断信息" }))
			end
